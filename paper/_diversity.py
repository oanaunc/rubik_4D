import time, random, math, json, os
...
# orbit/piece structure
from collections import defaultdict, Counter
def ccount(p): return sum(1 for c in p if c!=0)
piece_stickers=defaultdict(list)
for idx,(p,axis) in enumerate(M.stickers):
    piece_stickers[p].append(idx)
orbit_pieces={2:[],3:[],4:[]}
...
def run_div(model, target=7, n_accept=100000, seed=7):
    random.seed(seed)
    accepted=0; draws=0
    states=set(); dup=0
    d2=[]; d3=[]; d4=[]
    ...
    while accepted<n_accept:
        ...
        draws+=1
        d=exact_distance(s,target)
        if d!=target: continue
        accepted+=1
        if s in states: dup+=1
        else: states.add(s)
        n2=sum(1 for idxs in orbit_pieces[2] if any(s[i]!=i for i in idxs))
        n3=sum(1 for idxs in orbit_pieces[3] if any(s[i]!=i for i in idxs))
        n4=sum(1 for idxs in orbit_pieces[4] if any(s[i]!=i for i in idxs))
        d2.append(n2); d3.append(n3); d4.append(n4)
        disp_total.append(sum(1 for i in range(216) if s[i]!=i))
        sigs[(n2,n3,n4)]+=1
    def stats(a): return dict(mean=round(sum(a)/len(a),2), min=min(a), max=max(a))
    N=accepted
    H=-sum((c/N)*math.log2(c/N) for c in sigs.values())
    return dict(model=model, accepted=accepted, draws=draws,
        accept_rate=round(accepted/draws,4),
        duplicates=dup, unique=len(states),
        disp2=stats(d2), disp3=stats(d3), disp4=stats(d4),
        disp_total=stats(disp_total),
        distinct_signatures=len(sigs), signature_entropy_bits=round(H,2),
        effective_support=round(2**H,1))

out={}
for model in ("nii","nsf"):
    t0=time.perf_counter()
    out[model]=run_div(model, 7, 100000, seed=7 if model=="nii" else 8)
    out[model]["seconds"]=round(time.perf_counter()-t0,1)
    print(json.dumps(out[model],indent=1),flush=True)
