...
L=int(sys.argv[1]); Ms=int(sys.argv[2]); use_np="--np" in sys.argv
random.seed(1000+L); sd=sd2=0.0; red=0
t0=time.perf_counter()
for _ in range(Ms):
    s=ident
    for _ in range(L): s=s.translate(H[random.randrange(G)])
    d=dist_np(np.frombuffer(s,dtype=np.uint8).copy(),L) if use_np else dist_py(s,L)
    sd+=d; sd2+=d*d
    if d<L: red+=1
mean=sd/Ms; se=math.sqrt(max(0.,sd2/Ms-mean*mean)/Ms); p=red/Ms
out=dict(L=L,M=Ms,R=R,method=("numpy" if use_np else "python"),
    mean=round(mean,4),se=round(se,4),eff=round(mean/L,4),
    red=round(p,4),red_wilson=wilson(p,Ms),inverse_only=round(L-(L-1)/36,4),
    seconds=round(time.perf_counter()-t0,1))
json.dump(out,open("/tmp/htm_L%d.json"%L,"w"),indent=1)
